...
exports.post_create_fcs = [
    ...
    (0, express_validator_1.body)('FeeConfigurationSpec.*.FEE_LOCALE').isString().withMessage('FEE_LOCALE must be a string').matches(/^(LOCL|INTL|\*)$/).withMessage('FEE_LOCALE must be one of "LOCL" or "INTL" or "*"'),
    (0, express_validator_1.body)('FeeConfigurationSpec.*.FEE_ENTITY').isString().withMessage('FEE_ENTITY must be a string').matches(/^(CREDIT-CARD|DEBIT-CARD|BANK-ACCOUNT|USSD|WALLET-ID|\*)$/).withMessage('FEE_ENTITY must be one of CREDIT-CARD, DEBIT-CARD, BANK-ACCOUNT, USSD, WALLET-ID, or *'),
    ...
    async (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            try {
                ...
                const found_fcs = await FeeConfigSpec_1.default.find({});
                let errorArray = [];
                FeeConfigurationSpec.forEach((spec) => {
                    const found_fc = found_fcs.find(fc => fc.FEE_ID === spec.FEE_ID);
                    if (found_fc)
                        errorArray = [...errorArray, spec.FEE_ID];
                });
                if (errorArray.length)
                    throw new AppError_1.AppError(`FeeConfigurationSpec with FEE_ID(s) ${errorArray.join(', ')} already exists`, 400);
                const fcs = await FeeConfigSpec_1.default.create(FeeConfigurationSpec);
