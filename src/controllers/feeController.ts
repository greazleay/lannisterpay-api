...
export const post_create_fcs = [
    ...
    body('FeeConfigurationSpec.*.FEE_LOCALE').isString().withMessage('FEE_LOCALE must be a string').isLength({ min: 4, max: 4 }).withMessage('FEE_LOCALE must be a non-empty string of length 3').matches(/^(LOCL|INTL)$/).withMessage('FEE_LOCALE must be one of "LOCL" or "INTL"'),
    body('FeeConfigurationSpec.*.FEE_ENTITY').isString().withMessage('FEE_ENTITY must be a string').matches(/^(CREDIT-CARD|DEBIT-CARD|BANK-ACCOUNT|USSD|WALLET-ID)$/).withMessage('FEE_ENTITY must be one of CREDIT-CARD, DEBIT-CARD, BANK-ACCOUNT, USSD, WALLET-ID'),
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!errors.isEmpty()) {
            ...
            try {
                ...
                const found_fcs = await FCS.find({});
                ...
                const fcs = await FCS.create(FeeConfigurationSpec);
